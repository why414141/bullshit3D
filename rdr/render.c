...
pointstate  pointInTriangle(Vec3D * triangle, Vec3D * p){
	//fuck all code that was here before	
	//punk goes hard af with porn
	float max_x = -1000.0;
	float min_x = 1000.0;
	float max_y = -1000.0;
	float min_y = 1000.0;
...
int drawLine_(Vec3D *p1, Vec3D *p2, Buffer * buffer, char color, tiltspec state, int n){
		...
		if (abs(slope) > 1||deltaX == 0.0){
			...
			//function always returns 0 but for cleanliness reasons it's int
			//in bullshit cases ends up funking up shit
		}
